...
// domain (it's under-filled by ceil rounding). Falls back to byte-balanced fill when domain_layers==0 (no
// .orkpack index / layer unknown). `layer` = ork_layer_of(name), or -1 if non-layer/unknown.
...
ork_resolve_weight_i8(ggml_backend_ork_context * ctx, const struct ggml_tensor * src0,
    ...
    if (ctx->persist_mode == 1) {
        ...
        if (pit != ctx->persist_idx.end() && pit->second.K == (uint32_t) K && pit->second.N == (uint32_t) N &&
            ...
            int _dom = ork_weight_domain(ctx, (size_t) K * N, ork_layer_of(src0->name));   // layer-aligned (or byte-balanced) residence
